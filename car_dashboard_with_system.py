...
class CarDashboard(BoxLayout):
    def __init__(self, **kwargs):
        ...
        sys_btn = Button(text="System", font_size=26)
    ...
    def open_system(self, *_):
        """開発段階：アプリの再起動/終了だけ提供（実機導入後にOS再起動など拡張可）"""
        content = BoxLayout(orientation="vertical", spacing=10, padding=12)

        btn_row = BoxLayout(spacing=10, size_hint_y=None, height=56)
        restart_btn = Button(text="アプリ再起動", font_size=22)
        quit_btn = Button(text="アプリ終了", font_size=22)
        btn_row.add_widget(restart_btn)
        btn_row.add_widget(quit_btn)

        back_btn = Button(text="戻る", font_size=22, size_hint_y=None, height=50)
    ...
    def _restart_app(self):
        # ポップアップを閉じてから再起動
        try:
            if hasattr(self, "_system_popup"):
                self._system_popup.dismiss()
        except Exception:
            pass

        # 今のPythonプロセスを同じ引数で再実行
